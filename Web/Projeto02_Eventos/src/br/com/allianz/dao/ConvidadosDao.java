...
public class ConvidadosDao extends Dao<Convidado, String> {
	...
	public void Incluir(Convidado elemento) throws Exception {
		...
		try {			
			...
			stmt = cn.prepareStatement("INSERT INTO CONVIDADOS (CPF, IDEVENTO, NOME, EMAIL, TELEFONE) "
					+ "VALUES (?, ?, ?, ?, ?) ");
	...
	public Convidado Buscar(String chave) throws Exception {
		...
		try {
			...
			stmt = cn.prepareStatement("SELECT * FROM CONVIDADO WHERE ID=?");
	...
	public List<Convidado> ListarConvidados(int... id) throws Exception{
		...
		try {
			...
			if(id.length > 1) {
				throw new IllegalArgumentException("Este método admite no máximo UM parâmetro.");
			}
